...
function checkFile(fullPath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (nonAsciiChar) {
      ...
      console.error(
        `\x1b[31mError:\x1b[0m Non-ASCII character '\x1b[33m${char}\x1b[0m' found in ${relativePath}:${i + 1}`,
      );
...
if (hasError) {
  console.error(
    `\x1b[31mLint failed:\x1b[0m Non-ASCII characters are not allowed to enforce i18n.`,
  );
  process.exit(1);
} else {
  console.log(
    `\x1b[32mSuccess:\x1b[0m All scanned files contain only ASCII characters.`,
  );
  process.exit(0);
}
