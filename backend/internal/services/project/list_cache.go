...
import (
	...
	projectcache "github.com/kurodakayn/mpp-backend/internal/services/project/cache"
)

func (s *Service) projectListCache() *projectcache.Cache {
	if s == nil {
		return nil
	}
	return projectcache.New(projectcache.Config{
		Client: s.cache,
		TTL:    s.cacheTTL,
		Group:  s.cacheGroup,
...
func (s *Service) getCachedDashboardProjectList(cursor string, page, limit int, status, filterUserID, platform string, scopeUserID *uuid.UUID) (*dto.PaginationResponse, error) {
	params := projectcache.Params{
		...
		Platform:     platform,
		ScopeUserID:  projectcache.UUIDStringValue(scopeUserID),
	}
