...
当前总体进度：约 `35%`。这个数字按阶段权重人工估算，后续可按实际完成项调整。
...
| 阶段 2：读模型与缓存优先                     | 15%  | 56%        | 进行中 | Redis、Asynq 基础依赖可复用；admin dashboard stats、admin project list、dashboard account 摘要已有短 TTL Redis 缓存；stats/project list/account 缓存 miss 已用 singleflight 合并；项目创建、编辑、内容保存和平台保存后会清理 admin project list 缓存 | dashboard 读模型、stats/prepublish/publish 缓存失效、读模型重建任务                    |
...
| Redis 读缓存       | 进行中   | Redis 已用于队列、锁、OAuth、browser session 等短期协调；admin dashboard stats、admin project list 与 dashboard account 摘要已接入 15s TTL 缓存，并绕过 scoped/sticky writer 强一致路径；stats/project list/account 缓存 miss 已用 singleflight 做进程内防击穿；stats 和 account 缓存使用版本化 payload 和语义校验，Redis 读错误 fallback 也会合并为同 key 单次 DB 计算；平台账号保存、连接测试、X OAuth2 成功回调和浏览器 Cookie 登录完成后会删除对应工作区账号缓存；项目创建、编辑、内容保存和平台保存后会通过 SCAN 清理 admin project list 缓存 | stats、prepublish、publish 写路径的缓存失效，以及 dashboard 读模型与全量重建任务未实现 | `backend/internal/services/stats/overview.go`、`backend/internal/services/stats/overview_test.go`、`backend/internal/services/project/list_cache.go`、`backend/internal/services/project/list_cache_test.go`、`backend/internal/services/platform_account/account_cache.go`、`backend/internal/services/platform_account/account_cache_test.go`、`backend/internal/services/browser_session/complete.go`、`backend/internal/services/browser_session/service_test.go` |
