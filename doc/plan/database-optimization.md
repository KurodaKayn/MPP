...
当前总体进度：约 `30%`。这个数字按阶段权重人工估算，后续可按实际完成项调整。
...
| 阶段 2：读模型与缓存优先                     | 15%  | 45%        | 进行中 | Redis、Asynq 基础依赖可复用；admin dashboard stats、admin project list、dashboard account 摘要已有短 TTL Redis 缓存；project list/account 缓存 miss 已用 singleflight 合并 | dashboard 读模型、全局精细失效策略、读模型重建任务                                    |
