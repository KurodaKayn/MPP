...
当前总体进度：约 `58%`。这个数字按阶段权重人工估算，后续可按实际完成项调整。
...
| 阶段 1：单库连接池、索引、分页和生命周期治理 | 15%  | 100%       | 完成   | backend/publish-worker/collab-service 应用层连接池、Redis 客户端连接池、PgBouncer writer pool、组合索引、keyset 列表分页、列表避开 `source_content` 大字段、事件/会话历史保留期、R2/S3 冷事件归档 worker | 无；阶段 2 已完成，后续进入阶段 3/4 的读副本、分区和恢复流程                          |
| 阶段 2：读模型与缓存优先                     | 15%  | 100%       | 完成   | Redis、Asynq 基础依赖可复用；admin dashboard stats、admin project list、dashboard account 摘要已有短 TTL Redis 缓存；stats/project list/account 缓存 miss 已用 singleflight 合并；项目、预发布、发布和账号写路径已清理对应 dashboard 缓存；`workspace_dashboard_stats` 与 `project_list_summaries` 读模型已落地，API 在覆盖完整时优先读取读模型；项目保存、平台同步、发布完成、成员变更后触发异步刷新；admin rebuild API、Asynq 队列与 worker 已支持读模型全量重建 | 无                                                                                     |
| 阶段 3：读写分离                             | 15%  | 100%       | 完成   | `DB_READER_*` 可选连接、应用层 DB Router、签名 sticky writer、project/stats/workspace/platform_account/publish/prepublish/mediaasset/browser_session/extension 一致性路由、dashboard/publish/collab-service 一致性等级清单、self-hosted PostgreSQL read replica、managed `postgres-reader` 接入点、PgBouncer reader pool、replica lag 监控与超阈值回退 writer | 无；阶段 4 继续推进分区、归档和恢复流程                                                |
...
- [x] 增加读模型全量重建任务。验证入口：`backend/internal/services/readmodel/service.go`、`backend/internal/services/readmodel/queue.go`、`backend/internal/handlers/dashboard.go`、`backend/cmd/api/main.go`、`backend/cmd/publish-worker/main.go`、`backend/internal/services/readmodel/service_test.go`、`backend/internal/services/readmodel/queue_test.go`。

#### 阶段 3：读写分离

- [x] 部署 PostgreSQL read replica。验证入口：`deploy/kubernetes/data-services/self-hosted/postgres.yaml`、`deploy/kubernetes/data-services/managed/services.yaml`、`deploy/kubernetes/overlays/staging-managed/kustomization.yaml`、`deploy/kubernetes/overlays/production-managed/kustomization.yaml`。
...
- [x] 补齐 publish、prepublish、mediaasset、browser_session、extension 等剩余服务的一致性路由。验证入口：`backend/internal/services/publish`、`backend/internal/services/prepublish`、`backend/internal/services/mediaasset`、`backend/internal/services/browser_session`、`backend/internal/services/extension`。
- [x] 增加 PgBouncer reader pool，并让自托管 app baseline 使用 reader pool。验证入口：`deploy/kubernetes/data-services/self-hosted/pgbouncer.yaml`、`deploy/kubernetes/app-baseline/app-config.yaml`。
...
| publish | 发布前读取项目、publication、平台账号、媒体资源可用性 | `read-your-write` | 已通过 DB Router 走 `StrongRead`/sticky writer；发布凭据、媒体引用和 publication 状态机不走副本 | `backend/internal/services/publish/service.go`、`backend/internal/services/publish/media_refs.go` |
| publish | 发布排队、幂等检查、publish event、outbox event、publication 状态更新、retry/claim | `writer` | 已通过 DB Router 走 `Writer`；事务 outbox 和状态机不得走 reader | `backend/internal/services/publish/queue.go`、`backend/internal/services/publish/outbox.go` |
| publish | 发布历史、低优先级审计报表、增长分析 | `analytics-read` | 当前不是独立读模型；阶段 4/Outbox 后可迁移到 reader/归档查询 | `backend/internal/models/models.go` |
| prepublish | 项目、publication 和草稿同步前置读取 | `read-your-write` | 已通过 DB Router 走 `StrongRead`，避免用户刚保存后副本未追上导致同步旧内容 | `backend/internal/services/prepublish/drafts.go` |
| prepublish | 草稿同步、publication 更新和缓存失效触发 | `writer` | 已通过 DB Router 走 `Writer`，事务内读取不走副本 | `backend/internal/services/prepublish/drafts.go` |
| mediaasset | 上传完成、删除、对象存储状态更新 | `writer` | 已通过 DB Router 走 `Writer`，元数据状态变更不走副本 | `backend/internal/services/mediaasset/assets.go` |
| mediaasset | 媒体只读解析、展示读取 | `eventual-read` | 已通过 DB Router 走 `EventualRead`，副本 lag 不健康时自动回退 writer | `backend/internal/services/mediaasset/assets.go` |
| browser_session | live session、stream token、callback 状态读取和完成写入 | `read-your-write` / `writer` | 已通过 DB Router 将活跃会话读取固定为 `StrongRead`，注册/完成/清理写路径固定为 `Writer` | `backend/internal/services/browser_session/service.go`、`backend/internal/services/browser_session/complete.go` |
| extension | callback token、handoff 创建、执行事件、账号/session 校验 | `read-your-write` / `writer` | 已通过 DB Router 将 token/session 校验走 `StrongRead`，handoff 与事件写入走 `Writer` | `backend/internal/services/extension/handoffs.go` |
| extension | 预发布候选列表展示 | `eventual-read` | 已通过 DB Router 走 `EventualRead`，允许短延迟并受 replica lag 回退保护 | `backend/internal/services/extension/handoffs.go` |
