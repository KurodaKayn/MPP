...
Current overall progress: about `58%`. This number is manually estimated by phase weight and can be adjusted later according to actual completed items.
...
- [ ] Partition `publish_events`, `extension_execution_events`, and activity tables by month.
- [ ] Partition `remote_browser_sessions` by time or expiration time.
- [ ] Hash partition `collab_document_update_batches` by `document_id`.
- [ ] Export cold partitions to R2/S3.
