...
当前总体进度：约 `33%`。这个数字按阶段权重人工估算，后续可按实际完成项调整。
...
| 读写分离           | 进行中   | 已支持 `DB_READER_*` 可选读副本连接、`DefaultRouter`、签名 sticky writer；project/stats/workspace/platform_account 已接入 strong/eventual 路由和 stale replica 回归测试；writer pool 已在 Docker Compose 和自托管 Kubernetes 落地 | 生产 read replica、PgBouncer reader pool、剩余服务一致性标注、replica lag fallback 未实现 | `backend/internal/db/db.go`、`backend/internal/db/router.go`、`backend/internal/middleware/sticky_writer.go`、`deploy/kubernetes/data-services/self-hosted/pgbouncer.yaml` |
