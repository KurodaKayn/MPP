...
当前总体进度：约 `17%`。这个数字按阶段权重人工估算，后续可按实际完成项调整。

| 阶段                                         | 权重 | 当前完成度 | 状态   | 已完成                                                                         | 未完成/下一步                                                                          |
| -------------------------------------------- | ---- | ---------- | ------ | ------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------- |
| 阶段 0：数据层基线盘点                       | 10%  | 65%        | 进行中 | GORM 查询观测、`mpp_db_*` 指标、dashboard 查询计划审计脚本、`pg_stat_statements`、数据库基线审计脚本、DB 访问一致性 inventory | 表增长面板、版本化迁移规范                                                            |
...
| DB 访问一致性分类  | 完成     | 已按 dashboard、publish、collab-service 标注 `WriterOnly`、`ReadYourWrite`、`EventualRead`、`AnalyticsRead`，并记录当前路由和后续目标路由 | 还未落地代码级 DB Router、reader/writer 双连接、路由指标              | `doc/plan/database-access-consistency-inventory.md`                                      |
...
- [x] 给 dashboard、publish、collab-service 的 DB 调用标注一致性等级：`doc/plan/database-access-consistency-inventory.md`。
...
- [ ] 为 dashboard stats/list/account 摘要增加 Redis TTL 缓存。
- [ ] 增加读模型全量重建任务。
