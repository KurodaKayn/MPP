...
| K8S-012 | Define Kubernetes service manifests | Kubernetes deployment | Done | 100% | DevOps | `deploy/kubernetes/app-baseline` defines renderable Deployment/Service bases for frontend, backend, publish-worker, browser-worker, ai-service, content-pipeline-service, and collab-service, with probes, resources, PDBs, HPAs, restricted workload security contexts, and app namespace default-deny plus public/internal ingress allowlist NetworkPolicies; `deploy/kubernetes/overlays/staging-self-hosted`, `deploy/kubernetes/overlays/staging-managed`, and `deploy/kubernetes/overlays/production-managed` combine the app baseline with runtime controls and data services as renderable environment starters. |
...
| K8S-014 | Define Kubernetes ConfigMaps and Secrets | Configuration | Done | 100% | DevOps | `deploy/kubernetes/app-baseline/app-config.yaml` captures non-secret app, data-service, object-storage, LLM, collaboration, and X OAuth2 configuration placeholders; app workloads reference the complete required `mpp-app-secrets` contract for JWT, database, collaboration, cookie encryption, LLM, internal service, R2, and X OAuth2 credentials; `deploy/kubernetes/validation/app-baseline` and staging overlays supply renderable fake Secret material; `deploy/kubernetes/external-secrets` defines a provider-neutral External Secrets Operator contract for production `mpp-app-secrets`; `deploy/kubernetes/overlays/production-managed` renders that ExternalSecret while rejecting checked-in raw Secrets; Ruby validation checks ConfigMap/Secret keys against `contracts/env.schema.yaml`, and the Kubernetes smoke harness verifies required ConfigMap and Secret keys. |
| K8S-015 | Support managed PostgreSQL configuration | Data services | Done | 100% | DevOps / Backend | `deploy/kubernetes/data-services/managed` provides managed PostgreSQL writer and reader ExternalName Services plus guidance for provider DNS, TLS, credentials, and custom CA bundles; backend and collab-service support `DB_SSLMODE` and `DB_SSLROOTCERT`, backend supports optional `DB_READER_*` read replicas, and the staging-managed and production-managed overlays validate provider-host `DB_HOST`/`DB_READER_HOST` with `DB_SSLMODE=verify-full`. |
...
| K8S-021 | Publish images to a registry | Release | Done | 100% | DevOps | `.github/workflows/container-images.yml` publishes all service and browser runtime images to GHCR on `main`, `v*` tags, and manual dispatches with immutable `sha-<full-git-sha>` tags; pull requests build every image without pushing when service image inputs change; `script/kubernetes/pin-overlay-images.rb` pins app and browser runtime images in production overlays to one promoted Git SHA with configurable provider image namespaces; `.github/workflows/kubernetes-image-promotion.yml` verifies promoted tags, runs provider-specific image promotion and deployable overlay validation, and emits a reviewable promotion patch artifact. |
...
| K8S-023 | Add Kubernetes production setup guide | Documentation | Done | 100% | Documentation | `doc/setup-kubernetes.md` documents prerequisites, overlays, provider-specific production overlay patterns for AWS/GCP/Azure, secrets, data services, self-hosted backup starters, runtime selection, ingress namespace labeling, app NetworkPolicy boundaries, trusted metrics namespace L4 boundaries, observability, validation, staging-managed, staging-self-hosted, production-managed strict validation, deployment flow, smoke testing, and operational handoff. |
