...
| K8S-012 | Define Kubernetes service manifests | Kubernetes deployment | Partially Done | 95% | DevOps | `deploy/kubernetes/app-baseline` defines renderable Deployment/Service bases, PDBs, HPAs, restricted workload security contexts, and app NetworkPolicy; `deploy/kubernetes/overlays/staging-self-hosted` and `deploy/kubernetes/overlays/staging-managed` combine the app baseline with runtime controls and data services as renderable staging starters; production/provider-specific hardening remains future work. |
...
| K8S-022 | Add manifest validation in CI | Release quality | Partially Done | 92% | DevOps | CI detects `deploy/kubernetes` changes, builds every Kustomize package, and runs a split Ruby validator that rejects local/latest app images while checking app workload security, probes, resources, Services, Ingress, browser runtime RBAC/NetworkPolicy/admission policy, observability rules, data-service package shape, and staging overlay invariants for managed and self-hosted modes; deployable staging validation can additionally reject example hosts, all-zero image tags, and example Secret values; external schema validation remains future work. |
| K8S-023 | Add Kubernetes production setup guide | Documentation | Partially Done | 85% | Documentation | `doc/setup-kubernetes.md` documents prerequisites, overlays, secrets, data services, runtime selection, observability, validation, staging-managed and staging-self-hosted strict validation, and deployment flow; production provider-specific overlays remain future work. |
...
| K8S-025 | Run end-to-end Kubernetes smoke test | Verification | Partially Done | 45% | QA / DevOps | `script/kubernetes/smoke-test.rb` adds a live-cluster smoke harness for rollout status, Service endpoints, config and Secret shape, internal readiness, publish-worker dependencies, runtime RBAC, runtime cleanup metadata, public frontend readiness, optional authenticated dashboard/project probes, optional collaboration session probes, and optional remote browser session start/cancel; the script still needs execution against a real staging cluster. |
