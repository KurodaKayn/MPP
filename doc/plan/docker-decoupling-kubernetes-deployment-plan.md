...
| K8S-022 | Add manifest validation in CI | Release quality | Partially Done | 70% | DevOps | CI detects `deploy/kubernetes` changes, builds every Kustomize package, rejects local/latest app images, and validates app baseline dependencies, ingress hosts, and PostgreSQL TLS config; stronger schema/policy validation remains future work. |
...
| K8S-024 | Add Kubernetes operations runbooks | Operations | Partially Done | 55% | Documentation / DevOps | `doc/setup-kubernetes.md` includes initial rollback, runtime cleanup, scaling, secret rotation, backup responsibility notes, and observability alert coverage; deeper incident runbooks remain future work. |
