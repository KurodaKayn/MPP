...
| K8S-012 | Define Kubernetes service manifests | Kubernetes deployment | Partially Done | 85% | DevOps | `deploy/kubernetes/app-baseline` defines renderable Deployment/Service bases, PDBs, HPAs, restricted workload security contexts, and app NetworkPolicy; deployable environment overlays remain future work. |
...
| K8S-014 | Define Kubernetes ConfigMaps and Secrets | Configuration | Partially Done | 45% | DevOps | `deploy/kubernetes/app-baseline/app-config.yaml` captures non-secret config placeholders, including PostgreSQL TLS policy; `deploy/kubernetes/validation/app-baseline` supplies fake CI-only Secret material; real Secret materialization and environment-specific overlays remain future work. |
...
| K8S-017 | Add optional in-cluster PostgreSQL manifests | Data services | Partially Done | 70% | DevOps | `deploy/kubernetes/data-services/self-hosted` includes a restricted PostgreSQL StatefulSet for test clusters and small installations; backup tooling and TLS overlays remain future work. |
| K8S-018 | Add optional in-cluster Redis manifests | Data services | Partially Done | 70% | DevOps | `deploy/kubernetes/data-services/self-hosted` includes a restricted Redis StatefulSet for test clusters and small installations; backup and production hardening overlays remain future work. |
...
| K8S-022 | Add manifest validation in CI | Release quality | Partially Done | 85% | DevOps | CI detects `deploy/kubernetes` changes, builds every Kustomize package, and runs a split Ruby validator that rejects local/latest app images while checking app workload security, probes, resources, Services, Ingress, browser runtime RBAC/NetworkPolicy/admission policy, observability rules, and data-service package shape; external schema validation remains future work. |
| K8S-023 | Add Kubernetes production setup guide | Documentation | Partially Done | 75% | Documentation | `doc/setup-kubernetes.md` documents prerequisites, overlays, secrets, data services, runtime selection, observability, validation, and deployment flow; provider-specific overlays remain future work. |
