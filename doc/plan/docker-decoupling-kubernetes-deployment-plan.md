...
| K8S-021 | Publish images to a registry | Release | Partially Done | 65% | DevOps | `.github/workflows/container-images.yml` publishes all service and browser runtime images to GHCR on `main`, `v*` tags, and manual dispatches with immutable `sha-<full-git-sha>` tags; environment overlays and promotion policy remain future work. |
