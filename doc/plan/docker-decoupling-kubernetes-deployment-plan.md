...
| K8S-012 | Define Kubernetes service manifests | Kubernetes deployment | Partially Done | 65% | DevOps | `deploy/kubernetes/app-baseline` defines renderable Deployment/Service bases for frontend, backend, publish-worker, browser-worker, ai-service, content-pipeline-service, and collab-service; deployable environment overlays, PDBs, and HPAs remain future work. |
| K8S-013 | Define Kubernetes ingress or gateway | Kubernetes deployment | Not Started | 0% | DevOps | Replaces Traefik Docker labels. |
| K8S-014 | Define Kubernetes ConfigMaps and Secrets | Configuration | Partially Done | 20% | DevOps | `deploy/kubernetes/app-baseline/app-config.yaml` captures non-secret config placeholders and `deploy/kubernetes/validation/app-baseline` supplies fake CI-only Secret material; real Secret materialization and environment-specific overlays remain future work. |
| K8S-015 | Support managed PostgreSQL configuration | Data services | Not Started | 0% | DevOps / Backend | Production Kubernetes should prefer managed PostgreSQL. |
| K8S-016 | Support managed Redis configuration | Data services | Not Started | 0% | DevOps / Backend | Production Kubernetes should prefer managed Redis. |
| K8S-017 | Add optional in-cluster PostgreSQL manifests | Data services | Not Started | 0% | DevOps | Useful for test clusters and small self-hosted installations. |
| K8S-018 | Add optional in-cluster Redis manifests | Data services | Not Started | 0% | DevOps | Useful for test clusters and small self-hosted installations. |
...
| K8S-022 | Add manifest validation in CI | Release quality | Partially Done | 60% | DevOps | CI detects `deploy/kubernetes` changes, builds every Kustomize package, rejects local/latest app images, and validates an app baseline dependency overlay; stronger schema/policy validation remains future work. |
