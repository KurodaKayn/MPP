...
| K8S-012 | Define Kubernetes service manifests | Kubernetes deployment | Partially Done | 90% | DevOps | `deploy/kubernetes/app-baseline` defines renderable Deployment/Service bases, PDBs, HPAs, restricted workload security contexts, and app NetworkPolicy; `deploy/kubernetes/overlays/staging-self-hosted` now combines the app baseline with runtime controls and self-hosted data services as a renderable staging starter; production/provider overlays remain future work. |
...
| K8S-014 | Define Kubernetes ConfigMaps and Secrets | Configuration | Partially Done | 60% | DevOps | `deploy/kubernetes/app-baseline/app-config.yaml` captures non-secret config placeholders, including PostgreSQL TLS policy; app workloads now reference required browser-worker and AI internal-token Secret keys; `deploy/kubernetes/validation/app-baseline` supplies fake CI-only Secret material; `deploy/kubernetes/overlays/staging-self-hosted` supplies a renderable self-hosted staging ConfigMap and example Secret generator; real secret-manager materialization remains future work. |
| K8S-015 | Support managed PostgreSQL configuration | Data services | Partially Done | 70% | DevOps / Backend | `deploy/kubernetes/data-services/managed` provides PostgreSQL managed-service guidance, and backend/collab-service support `DB_SSLMODE` and `DB_SSLROOTCERT`; provider-specific overlays remain future work. |
| K8S-016 | Support managed Redis configuration | Data services | Partially Done | 60% | DevOps / Backend | `deploy/kubernetes/data-services/managed` provides a Redis ExternalName and app config supports `REDIS_TLS`; provider-specific overlays remain future work. |
...
| K8S-022 | Add manifest validation in CI | Release quality | Partially Done | 90% | DevOps | CI detects `deploy/kubernetes` changes, builds every Kustomize package, and runs a split Ruby validator that rejects local/latest app images while checking app workload security, probes, resources, Services, Ingress, browser runtime RBAC/NetworkPolicy/admission policy, observability rules, data-service package shape, and staging-self-hosted overlay invariants; deployable staging validation can additionally reject example hosts, all-zero image tags, and example Secret values; external schema validation remains future work. |
| K8S-023 | Add Kubernetes production setup guide | Documentation | Partially Done | 80% | Documentation | `doc/setup-kubernetes.md` documents prerequisites, overlays, secrets, data services, runtime selection, observability, validation, staging-self-hosted strict validation, and deployment flow; provider-specific overlays remain future work. |
