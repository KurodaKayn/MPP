...
| K8S-014 | Define Kubernetes ConfigMaps and Secrets | Configuration | Partially Done | 65% | DevOps | `deploy/kubernetes/app-baseline/app-config.yaml` captures non-secret config placeholders, including PostgreSQL TLS policy; app workloads now reference required browser-worker, AI, and content-pipeline internal-token Secret keys; `deploy/kubernetes/validation/app-baseline` supplies fake CI-only Secret material; staging overlays supply renderable ConfigMaps and example Secret generators for managed and self-hosted data modes; real secret-manager materialization remains future work. |
