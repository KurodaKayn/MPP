...
| K8S-020 | Add runtime session metrics and alerts | Observability | Partially Done | 70% | Backend / DevOps | `browser-worker` exposes runtime lifecycle metrics, and `deploy/kubernetes/observability` defines startup failure, cleanup failure, and cleanup lag alerts; broader Redis/readiness/publish-worker alerts remain future work. |
...
| K8S-024 | Add Kubernetes operations runbooks | Operations | Partially Done | 45% | Documentation / DevOps | `doc/setup-kubernetes.md` includes initial rollback, runtime cleanup, scaling, secret rotation, backup responsibility notes, and runtime observability alerts; deeper incident runbooks remain future work. |
