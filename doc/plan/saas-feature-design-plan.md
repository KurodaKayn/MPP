...
- [x] 调整账号模型，支持 `workspace_id`、`owner_user_id`、`connected_by_user_id`、账号别名和远端账号 ID。
- [x] 迁移旧 `user_id + platform` 数据到个人工作区。
- [x] 建 `PlatformAccountGrant`，支持账号对成员或项目授权。
- [x] 远程浏览器登录会话绑定工作区和平台账号。
- [x] Cookie/credential 存储改为 secret ref，业务响应不返回敏感字段。
- [x] 账号管理页支持连接、测试、共享、重命名、重登、移除。
- [x] 发布前必须选择可用且已授权账号。
- [x] 账号异常统一展示并阻止对应发布。
...
- [x] 项目详情整合源内容、平台草稿、评论、版本、活动和发布结果。
- [x] 新增 `ContentTemplate`，支持系统、工作区、个人范围。
- [x] 新增 `BrandProfile`，绑定工作区，可被项目引用。
- [x] `Project` 增加 `template_id`、`brand_profile_id`。
- [x] `MediaAsset` 补齐 library scope、tags、alt text、source。
- [x] 新增 `MediaAssetUsage`，记录素材引用关系。
- [x] 平台草稿同步、编辑、审核状态清晰。
- [x] 版本恢复后提示重新同步平台草稿。
- [x] 分享链接和项目协作者权限与工作区权限合并展示。

验收：

- [x] 从模板创建项目可生成初始标题、正文结构和默认平台。
- [x] 项目可引用工作区素材，发布前能校验素材 ready 状态。
- [x] editor 能编辑内容但不能绕过发布权限。
- [x] viewer 只能查看内容、草稿和结果。
