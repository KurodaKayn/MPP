...
- [ ] 后端统一 `workspace_id` 上下文解析和权限校验。
- [ ] 前端 dashboard 路由和 API 请求统一携带当前工作区。
- [ ] 项目、账号、素材、活动、设置页全部按当前工作区过滤。
- [ ] 个人工作区自动创建和历史项目回填校验。
- [ ] 新增 `WorkspaceInvite`，支持邀请、接受、过期、撤销。
- [ ] 建立角色到权限点映射，补齐服务层 guard。
- [ ] 增加越权测试：跨工作区读取、编辑、发布、账号使用都必须拒绝。

验收：

- 切换工作区后，看不到其他工作区项目、账号、素材和排期。
- 无权限成员无法通过 API 修改成员、账号、发布和设置。
