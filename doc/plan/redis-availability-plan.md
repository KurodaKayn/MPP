...
Current overall progress: about `73%`.
...
| PR 4.3: Production migration runbook | Make cutover repeatable | Write exact steps in the [production migration runbook](../managed-redis-production-migration-runbook.md): precheck, freeze if needed, data copy, TTL diff, switch, monitor, rollback | Runbook dry-run completed and recorded against non-production or an isolated managed target | Revert runbook | No infra change |
| PR 4.4: Production managed Redis cutover | Move production to managed HA | Execute the [production migration runbook](../managed-redis-production-migration-runbook.md) and complete the [production cutover record](../managed-redis-production-cutover-record.md); monitor error rate, latency, memory, connection count, and cache hit rate | Production stable for agreed soak period with rollback and snapshot evidence retained | Switch endpoint back to self-hosted HA; restore snapshot if required | No Cluster sharding |
